...
var globalOptions = {
  confirmTasks: false,
  printDetails: false,
  duplicateItem: template_string_$item(lib_templateObject || (lib_templateObject = lib_taggedTemplateLiteral(["none"])))
};
...
function tryUse(item) {
  var quantity = arguments.length > 1 && arguments[1] !== undefined ? arguments[1] : 1;
  return have(item, quantity) && (0,external_kolmafia_namespaceObject.use)(quantity, item);
}
function isStealable(item) {
  return item.tradeable && item.discardable && !item.gift;
}
function isDMTDuplicable(item) {
  return isStealable(item) && ["food", "booze", "spleen item", "potion"].includes((0,external_kolmafia_namespaceObject.itemType)(item));
}
function organsFull() {
  return myFullness() >= fullnessLimit() && myInebriety() > inebrietyLimit() + (myFamiliar() !== $familiar(lib_templateObject2 || (lib_templateObject2 = lib_taggedTemplateLiteral(["Stooper"]))) ? 1 : 0) && mySpleenUse() >= spleenLimit();
}
function tuneMoon(moon) {
  if (!property_get("moonTuned") && (0,external_kolmafia_namespaceObject.mySign)().toLowerCase() !== moon.toLowerCase()) cliExecuteThrow("spoon ".concat(moon));
}
function canAscendNoncasual() {
  var sessionLog = (0,external_kolmafia_namespaceObject.fileToBuffer)("".concat((0,external_kolmafia_namespaceObject.myName)(), "_").concat((0,external_kolmafia_namespaceObject.todayToString)(), ".txt"));
  return !/Ascend as a (?:Normal|Hardcore) .+? banking \d+ Karma./.test(sessionLog);
}
function canAscendCasual() {
  var sessionLog = (0,external_kolmafia_namespaceObject.fileToBuffer)("".concat((0,external_kolmafia_namespaceObject.myName)(), "_").concat((0,external_kolmafia_namespaceObject.todayToString)(), ".txt"));
  return !/Ascend as a Casual .+? banking \d+ Karma./.test(sessionLog);
}
function ascensionsToday() {
  var sessionLog = (0,external_kolmafia_namespaceObject.fileToBuffer)("".concat((0,external_kolmafia_namespaceObject.myName)(), "_").concat((0,external_kolmafia_namespaceObject.todayToString)(), ".txt"));
...
function cliExecuteThrow(command) {
  if (!(0,external_kolmafia_namespaceObject.cliExecute)(command)) throw "Failed to execute ".concat(command);
}
...
var ascend_worksheds = (/* unused pure expression or super */ null && (["warbear LP-ROM burner", "warbear jackhammer drill press", "warbear induction oven", "warbear high-efficiency still", "warbear chemistry lab", "warbear auto-anvil", "spinning wheel", "snow machine", "Little Geneticist DNA-Splicing Lab", "portable Mayo Clinic", "Asdon Martin keyfob", "diabolic pizza cube", "cold medicine cabinet"]));
var ascend_gardens = (/* unused pure expression or super */ null && (["packet of pumpkin seeds", "Peppermint Pip Packet", "packet of dragon's teeth", "packet of beer seeds", "packet of winter seeds", "packet of thanksgarden seeds", "packet of tall grass seeds", "packet of mushroom spores"]));
...
function fillWithBestInventoryItem(targetUnits) {
  ...
  var amountToUse = (0,lodash.clamp)(Math.ceil(targetUnits / getAverageAdventures(best)), 0, (0,external_kolmafia_namespaceObject.itemAmount)(best));
...
var Driving = {
  ...
  Waterproofly: $effect(_templateObject14 || (_templateObject14 = AsdonMartin_taggedTemplateLiteral(["Driving Waterproofly"])))
};
/**
...
var tasks_templateObject, tasks_templateObject2, tasks_templateObject3, tasks_templateObject4, tasks_templateObject5, tasks_templateObject6, tasks_templateObject7, tasks_templateObject8, tasks_templateObject9;
...
function createGarboTask(ascend) {
  return {
    name: "Garbo",
    completed: () => (0,external_kolmafia_namespaceObject.myFullness)() >= (0,external_kolmafia_namespaceObject.fullnessLimit)() && (0,external_kolmafia_namespaceObject.myInebriety)() === (0,external_kolmafia_namespaceObject.inebrietyLimit)() && (0,external_kolmafia_namespaceObject.mySpleenUse)() >= (0,external_kolmafia_namespaceObject.spleenLimit)() && (0,external_kolmafia_namespaceObject.myAdventures)() === 0 || (0,external_kolmafia_namespaceObject.myInebriety)() > (0,external_kolmafia_namespaceObject.inebrietyLimit)(),
    do: () => {
      (0,external_kolmafia_namespaceObject.cliExecute)("breakfast");
      (0,external_kolmafia_namespaceObject.cliExecute)("Detective Solver.ash");
      tuneMoon("Platypus");

      if (ascend) {
        cliExecuteThrow("garbo yachtzeechain ascend");
      } else {
        cliExecuteThrow("garbo yachtzeechain");
      }

      (0,external_kolmafia_namespaceObject.cliExecute)("breakfast"); // Harvest sea jelly after garbo unlocks the sea
    }
  };
}

function createOverdrinkTask(ascend) {
  return {
    name: "Overdrink",
    completed: () => (0,external_kolmafia_namespaceObject.myInebriety)() > (0,external_kolmafia_namespaceObject.inebrietyLimit)(),
    do: () => {
      if (ascend) {
        ...
        (0,external_kolmafia_namespaceObject.cliExecute)("maximize adv switch tot"); // considerClockworkMaid();
      }
    }
  };
}

var garboAscendTask = createGarboTask(true);
var garboTask = createGarboTask(false);
var overdrinkAscendTask = createOverdrinkTask(true);
var overdrinkTask = createOverdrinkTask(false); // This task is a collection of misc actions so we track completion with a preference

var kingFreedProperty = "_fullday_kingFreed";
var kingFreedTask = {
  name: "King Freed",
  completed: () => property_get(kingFreedProperty, false),
  do: () => {
    (0,external_kolmafia_namespaceObject.cliExecute)("pull all");
    (0,external_kolmafia_namespaceObject.cliExecute)("peevpee.php?action=smashstone&confirm=on");
    (0,external_kolmafia_namespaceObject.cliExecute)("backupcamera reverser on");
    (0,external_kolmafia_namespaceObject.cliExecute)("closet put ".concat(Math.max(0, (0,external_kolmafia_namespaceObject.myMeat)() - 2000000), " meat"));
    tryUse(template_string_$item(tasks_templateObject || (tasks_templateObject = tasks_taggedTemplateLiteral(["can of Rain-Doh"]))));
    tryUse(template_string_$item(tasks_templateObject2 || (tasks_templateObject2 = tasks_taggedTemplateLiteral(["astral six-pack"]))));
    uneffect($effect(tasks_templateObject3 || (tasks_templateObject3 = tasks_taggedTemplateLiteral(["Feeling Lost"]))));
    _set(kingFreedProperty, true);
  }
};
var dmtDuplicateTask = {
  name: "DMT Duplicate",
  ready: () => (0,external_kolmafia_namespaceObject.myAdventures)() > 0 && (0,external_kolmafia_namespaceObject.itemAmount)(globalOptions.duplicateItem) > 0 && property_get("encountersUntilDMTChoice") < 1,
  completed: () => property_get("lastDMTDuplication") >= (0,external_kolmafia_namespaceObject.myAscensions)(),
  do: () => {
    (0,external_kolmafia_namespaceObject.useFamiliar)(template_string_$familiar(tasks_templateObject4 || (tasks_templateObject4 = tasks_taggedTemplateLiteral(["Machine Elf"]))));
    (0,external_kolmafia_namespaceObject.visitUrl)("adventure.php?snarfblat=458");

    if ((0,external_kolmafia_namespaceObject.handlingChoice)() && (0,external_kolmafia_namespaceObject.lastChoice)() === 1119) {
      (0,external_kolmafia_namespaceObject.visitUrl)("choice.php?pwd&whichchoice=1119&option=4");
      (0,external_kolmafia_namespaceObject.visitUrl)("choice.php?whichchoice=1125&pwd&option=1&iid=".concat((0,external_kolmafia_namespaceObject.toInt)(globalOptions.duplicateItem)));
    }
  }
};
var doghouseVolcoinoTask = {
  name: "Doghouse Volcoino",
  ready: () => (0,external_kolmafia_namespaceObject.myAdventures)() > 0,
  completed: () => $location(tasks_templateObject5 || (tasks_templateObject5 = tasks_taggedTemplateLiteral(["The Bubblin' Caldera"]))).noncombatQueue.includes("Lava Dogs"),
  do: () => {
    ensureEffect($effect(tasks_templateObject6 || (tasks_templateObject6 = tasks_taggedTemplateLiteral(["A Few Extra Pounds"]))));
    new Requirement(["mainstat"], {
      forceEquip: template_string_$items(tasks_templateObject7 || (tasks_templateObject7 = tasks_taggedTemplateLiteral(["Fourth of May Cosplay Saber, Mr. Screege's spectacles, mafia thumb ring, lucky gold ring"])))
    }).maximize();
    (0,external_kolmafia_namespaceObject.adv1)($location(tasks_templateObject8 || (tasks_templateObject8 = tasks_taggedTemplateLiteral(["The Bubblin' Caldera"]))), -1, Macro.attack().repeat().toString());
    if (have($effect(tasks_templateObject9 || (tasks_templateObject9 = tasks_taggedTemplateLiteral(["Beaten Up"]))))) throw "Fight was lost; stop.";
  },
  limit: 8
};
;// CONCATENATED MODULE: ./src/quests.ts
var quests_templateObject, quests_templateObject2, quests_templateObject3, quests_templateObject4, quests_templateObject5, quests_templateObject6, quests_templateObject7, quests_templateObject8, quests_templateObject9;
...
function _objectSpread(target) { for (var i = 1; i < arguments.length; i++) { var source = null != arguments[i] ? arguments[i] : {}; i % 2 ? ownKeys(Object(source), !0).forEach(function (key) { quests_defineProperty(target, key, source[key]); }) : Object.getOwnPropertyDescriptors ? Object.defineProperties(target, Object.getOwnPropertyDescriptors(source)) : ownKeys(Object(source)).forEach(function (key) { Object.defineProperty(target, key, Object.getOwnPropertyDescriptor(source, key)); }); } return target; }

function quests_defineProperty(obj, key, value) { if (key in obj) { Object.defineProperty(obj, key, { value: value, enumerable: true, configurable: true, writable: true }); } else { obj[key] = value; } return obj; }
...
var firstGarboQuest = {
  ...
  tasks: [_objectSpread({}, kingFreedTask), _objectSpread({}, garboAscendTask), _objectSpread({}, overdrinkAscendTask)]
};
var communityServiceQuest = {
  ...
  tasks: [{
    ...
    do: () => {
      ...
      ascend(Paths.CommunityService, template_string_$class(quests_templateObject || (quests_templateObject = quests_taggedTemplateLiteral(["Sauceror"]))), Lifestyle.softcore, "knoll", template_string_$item(quests_templateObject2 || (quests_templateObject2 = quests_taggedTemplateLiteral(["astral six-pack"]))), template_string_$item(quests_templateObject3 || (quests_templateObject3 = quests_taggedTemplateLiteral(["astral chapeau"]))));
    }
  }, {
    name: "Free King",
    ready: () => (0,external_kolmafia_namespaceObject.myPathId)() === Paths.CommunityService.id,
    completed: () => property_get("kingLiberated"),
    do: () => cliExecuteThrow("fizz-sccs.ash")
  }]
};
var secondGarboQuest = {
  name: "Second Garbo",
  completed: () => ascensionsToday() > 1,
  tasks: [_objectSpread({}, kingFreedTask), _objectSpread({}, dmtDuplicateTask), _objectSpread({}, doghouseVolcoinoTask), _objectSpread({}, garboAscendTask), _objectSpread({}, overdrinkAscendTask)]
};
var casualQuest = {
  name: "Casual",
  tasks: [{
    ...
    do: () => {
      ...
      ascend(Paths.Unrestricted, template_string_$class(quests_templateObject4 || (quests_templateObject4 = quests_taggedTemplateLiteral(["Seal Clubber"]))), Lifestyle.casual, "knoll", template_string_$item(quests_templateObject5 || (quests_templateObject5 = quests_taggedTemplateLiteral(["astral six-pack"]))));
    }
  }, {
    name: "Free King",
    ready: () => (0,external_kolmafia_namespaceObject.myPathId)() === Paths.Unrestricted.id,
    completed: () => property_get("kingLiberated") && have(template_string_$skill(quests_templateObject6 || (quests_templateObject6 = quests_taggedTemplateLiteral(["Liver of Steel"])))),
    do: () => cliExecuteThrow("loopcasual")
  }]
};
var thirdGarboQuest = {
  name: "Third Garbo",
  tasks: [_objectSpread({}, kingFreedTask), {
    name: "Workshed",
    completed: () => property_get("_workshedItemUsed") || (0,external_kolmafia_namespaceObject.getWorkshed)() === template_string_$item(quests_templateObject7 || (quests_templateObject7 = quests_taggedTemplateLiteral(["cold medicine cabinet"]))),
    do: () => {
      drive($effect(quests_templateObject8 || (quests_templateObject8 = quests_taggedTemplateLiteral(["Driving Observantly"]))), 1000);
      tryUse(template_string_$item(quests_templateObject9 || (quests_templateObject9 = quests_taggedTemplateLiteral(["cold medicine cabinet"]))));
    }
  }, _objectSpread({}, dmtDuplicateTask), _objectSpread({}, doghouseVolcoinoTask), _objectSpread({}, garboTask), _objectSpread({}, overdrinkTask)]
};
var quests = [firstGarboQuest, communityServiceQuest, secondGarboQuest, casualQuest, thirdGarboQuest];
function allTasks() {
  var result = [];

  var _iterator = quests_createForOfIteratorHelper(quests),
      _step;
  ...
  try {
    var _loop = function _loop() {
      ...
      var _iterator2 = quests_createForOfIteratorHelper(quest.tasks),
          _step2;
        ...
        for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
          ...
          if (questCompleted) {
            (function () {
              ...
              task.completed = () => taskCompleted() || questCompleted();
            })();
          }
...
function getEquipment() {
  var items = snapshot_toConsumableArray(new Set(external_kolmafia_namespaceObject.Slot.all().map(slot => (0,external_kolmafia_namespaceObject.equippedItem)(slot))));

  return items.reduce((obj, item) => {
    if ((0,external_kolmafia_namespaceObject.equippedAmount)(item) > 0) obj[item.name] = (0,external_kolmafia_namespaceObject.equippedAmount)(item);
    return obj;
  }, {});
}

function getTerrariumEquipment() {
  var items = external_kolmafia_namespaceObject.Familiar.all().filter(fam => have(fam) && (0,external_kolmafia_namespaceObject.myFamiliar)() !== fam).map(fam => (0,external_kolmafia_namespaceObject.familiarEquippedEquipment)(fam)).filter(item => item !== template_string_$item(snapshot_templateObject || (snapshot_templateObject = snapshot_taggedTemplateLiteral(["none"]))));
  return items.reduce((obj, item) => {
    var _obj$item$name;
...
function getTradeableCampground() {
  var itemNames = Object.keys((0,external_kolmafia_namespaceObject.getCampground)()).filter(x => [].concat(snapshot_toConsumableArray(worksheds), snapshot_toConsumableArray(gardens)).includes(x));
...
function myItems() {
  ...
  var inv = (0,external_kolmafia_namespaceObject.getInventory)();
  var equips = getEquipment();
  var shop = (0,external_kolmafia_namespaceObject.getShop)();
  var closet = (0,external_kolmafia_namespaceObject.getCloset)();
  var storage = (0,external_kolmafia_namespaceObject.getStorage)();
  var camp = getTradeableCampground();
  var terrarium = getTerrariumEquipment();
  var overall = new Map();

  var _loop = function _loop(itemName) {
    overall.set((0,external_kolmafia_namespaceObject.toItem)(itemName), [inv, equips, shop, closet, storage, camp, terrarium].reduce((a, b) => {
      var _b$itemName;
  ...
  for (var itemName in snapshot_objectSpread(snapshot_objectSpread(snapshot_objectSpread(snapshot_objectSpread(snapshot_objectSpread(snapshot_objectSpread(snapshot_objectSpread({}, inv), equips), shop), closet), storage), camp), terrarium)) {
    _loop(itemName);
  }
...
function myItemsWrapper() {
  ...
  var itemMappings = new Map([].concat(snapshot_toConsumableArray(foldable(template_string_$item(snapshot_templateObject2 || (snapshot_templateObject2 = snapshot_taggedTemplateLiteral(["liar's pants"]))))), snapshot_toConsumableArray(foldable(template_string_$item(snapshot_templateObject3 || (snapshot_templateObject3 = snapshot_taggedTemplateLiteral(["ice pick"]))))), snapshot_toConsumableArray(manyToOne(template_string_$item(snapshot_templateObject4 || (snapshot_templateObject4 = snapshot_taggedTemplateLiteral(["Spooky Putty sheet"]))), [template_string_$item(snapshot_templateObject5 || (snapshot_templateObject5 = snapshot_taggedTemplateLiteral(["Spooky Putty monster"])))].concat(snapshot_toConsumableArray(getFoldGroup(template_string_$item(snapshot_templateObject6 || (snapshot_templateObject6 = snapshot_taggedTemplateLiteral(["Spooky Putty sheet"])))))))), snapshot_toConsumableArray(foldable(template_string_$item(snapshot_templateObject7 || (snapshot_templateObject7 = snapshot_taggedTemplateLiteral(["stinky cheese sword"]))))), snapshot_toConsumableArray(foldable(template_string_$item(snapshot_templateObject8 || (snapshot_templateObject8 = snapshot_taggedTemplateLiteral(["naughty paper shuriken"]))))), snapshot_toConsumableArray(foldable(template_string_$item(snapshot_templateObject9 || (snapshot_templateObject9 = snapshot_taggedTemplateLiteral(["Loathing Legion knife"]))))), snapshot_toConsumableArray(foldable(template_string_$item(snapshot_templateObject10 || (snapshot_templateObject10 = snapshot_taggedTemplateLiteral(["deceased crimbo tree"]))))), snapshot_toConsumableArray(foldable(template_string_$item(snapshot_templateObject11 || (snapshot_templateObject11 = snapshot_taggedTemplateLiteral(["makeshift turban"]))))), snapshot_toConsumableArray(foldable(template_string_$item(snapshot_templateObject12 || (snapshot_templateObject12 = snapshot_taggedTemplateLiteral(["turtle wax shield"]))))), snapshot_toConsumableArray(foldable(template_string_$item(snapshot_templateObject13 || (snapshot_templateObject13 = snapshot_taggedTemplateLiteral(["metallic foil bow"]))))), snapshot_toConsumableArray(foldable(template_string_$item(snapshot_templateObject14 || (snapshot_templateObject14 = snapshot_taggedTemplateLiteral(["ironic moustache"]))))), snapshot_toConsumableArray(foldable(template_string_$item(snapshot_templateObject15 || (snapshot_templateObject15 = snapshot_taggedTemplateLiteral(["bugged balaclava"]))))), snapshot_toConsumableArray(foldable(template_string_$item(snapshot_templateObject16 || (snapshot_templateObject16 = snapshot_taggedTemplateLiteral(["toggle switch (Bartend)"]))))), snapshot_toConsumableArray(foldable(template_string_$item(snapshot_templateObject17 || (snapshot_templateObject17 = snapshot_taggedTemplateLiteral(["mushroom cap"]))))), snapshot_toConsumableArray(manyToOne(template_string_$item(snapshot_templateObject18 || (snapshot_templateObject18 = snapshot_taggedTemplateLiteral(["can of Rain-Doh"]))), template_string_$items(snapshot_templateObject19 || (snapshot_templateObject19 = snapshot_taggedTemplateLiteral(["empty Rain-Doh can"]))))), snapshot_toConsumableArray(manyToOne(template_string_$item(snapshot_templateObject20 || (snapshot_templateObject20 = snapshot_taggedTemplateLiteral(["meteorite fragment"]))), template_string_$items(snapshot_templateObject21 || (snapshot_templateObject21 = snapshot_taggedTemplateLiteral(["meteorite earring, meteorite necklace, meteorite ring"]))))), snapshot_toConsumableArray(manyToOne(template_string_$item(snapshot_templateObject22 || (snapshot_templateObject22 = snapshot_taggedTemplateLiteral(["Sneaky Pete's leather jacket"]))), template_string_$items(snapshot_templateObject23 || (snapshot_templateObject23 = snapshot_taggedTemplateLiteral(["Sneaky Pete's leather jacket (collar popped)"]))))), snapshot_toConsumableArray(manyToOne(template_string_$item(snapshot_templateObject24 || (snapshot_templateObject24 = snapshot_taggedTemplateLiteral(["Boris's Helm"]))), template_string_$items(snapshot_templateObject25 || (snapshot_templateObject25 = snapshot_taggedTemplateLiteral(["Boris's Helm (askew)"]))))), snapshot_toConsumableArray(manyToOne(template_string_$item(snapshot_templateObject26 || (snapshot_templateObject26 = snapshot_taggedTemplateLiteral(["Jarlsberg's pan"]))), template_string_$items(snapshot_templateObject27 || (snapshot_templateObject27 = snapshot_taggedTemplateLiteral(["Jarlsberg's pan (Cosmic portal mode)"]))))), snapshot_toConsumableArray(manyToOne(template_string_$item(snapshot_templateObject28 || (snapshot_templateObject28 = snapshot_taggedTemplateLiteral(["tiny plastic sword"]))), template_string_$items(snapshot_templateObject29 || (snapshot_templateObject29 = snapshot_taggedTemplateLiteral(["grogtini, bodyslam, dirty martini, vesper, cherry bomb, sangria del diablo"]))))), snapshot_toConsumableArray(manyToOne(template_string_$item(snapshot_templateObject30 || (snapshot_templateObject30 = snapshot_taggedTemplateLiteral(["earthenware muffin tin"]))), template_string_$items(snapshot_templateObject31 || (snapshot_templateObject31 = snapshot_taggedTemplateLiteral(["blueberry muffin, bran muffin, chocolate chip muffin"])))))));
...
var Snapshot = /*#__PURE__*/function () {
  ...
  snapshot_createClass(Snapshot, [{
    ...
    function toFile(filename) {
      ...
      (0,external_kolmafia_namespaceObject.bufferToFile)(JSON.stringify(val), Snapshot.getFilepath(filename));
    }
    /**
    ...
    value: function fromFile(filename) {
      var fileValue = (0,external_kolmafia_namespaceObject.fileToBuffer)(Snapshot.getFilepath(filename)); // fileToBuffer returns empty string for files that don't exist
    ...
    value: function current() {
      return new Snapshot((0,external_kolmafia_namespaceObject.myMeat)() + (0,external_kolmafia_namespaceObject.myStorageMeat)() + (0,external_kolmafia_namespaceObject.myClosetMeat)(), myItemsWrapper());
    }
  }, {
    key: "createOrImport",
    value: function createOrImport(filename) {
...
function main() {
  ...
  var duplicateProperty = "fullday_duplicateItem";
  if (!property_get(duplicateProperty)) throw "Set property \"".concat(duplicateProperty, "\" to the item you want to duplicate in the DMT");
  globalOptions.duplicateItem = (0,external_kolmafia_namespaceObject.toItem)(property_get(duplicateProperty));
  if (!isDMTDuplicable(globalOptions.duplicateItem)) throw "Cannot duplicate ".concat(globalOptions.duplicateItem, " in the DMT");
  ...
  var dateProperty = "fullday_runDate";
  var turnsProperty = "fullday_initialTurns";
  var timeProperty = "fullday_elapsedTime";

  if (property_get(dateProperty) !== (0,external_kolmafia_namespaceObject.todayToString)()) {
    _set(dateProperty, (0,external_kolmafia_namespaceObject.todayToString)());
    _set(turnsProperty, (0,external_kolmafia_namespaceObject.totalTurnsPlayed)());
    _set(timeProperty, (0,external_kolmafia_namespaceObject.gametimeToInt)());
  }

  var tasks = allTasks();
  var snapshots = [Snapshot.createOrImport("Start")];

  var _iterator2 = main_createForOfIteratorHelper(tasks),
      _step2;
  ...
  try {
    for (_iterator2.s(); !(_step2 = _iterator2.n()).done;) {
      ...
      while (!task.completed()) {
        ...
        var attempts = 0;
        if (attempts >= ((_task$limit = task.limit) !== null && _task$limit !== void 0 ? _task$limit : 1)) throw "Task ".concat(task.name, " did not complete within ").concat(task.limit, " attempts.");
        if (task.ready && !task.ready()) throw "Task ".concat(task.name, " is not ready");
        if (globalOptions.confirmTasks && !(0,external_kolmafia_namespaceObject.userConfirm)("Executing task ".concat(task.name, ". Should we continue?"))) throw "User rejected execution of task ".concat(task.name);
        task.do();
        attempts += 1;
      }

      uneffect($effect(main_templateObject || (main_templateObject = main_taggedTemplateLiteral(["Drenched in Lava"]))));
  ...
  Kmail["delete"](Kmail.inbox().filter(k => ["Lady Spookyraven's Ghost", "The Loathing Postal Service"].includes(k.senderName)));
  var fullSnapshot = snapshots[snapshots.length - 1].diff(snapshots[0]);
  var fullResult = fullSnapshot.value(external_kolmafia_namespaceObject.historicalPrice);
  (0,external_kolmafia_namespaceObject.print)("Full-day complete!", "purple");
  (0,external_kolmafia_namespaceObject.print)("Adventures used: ".concat(formatNumber((0,external_kolmafia_namespaceObject.totalTurnsPlayed)() - property_get(turnsProperty, (0,external_kolmafia_namespaceObject.totalTurnsPlayed)()))), "purple");
  (0,external_kolmafia_namespaceObject.print)("Time elapsed: ".concat(convertMilliseconds((0,external_kolmafia_namespaceObject.gametimeToInt)() - property_get(timeProperty, (0,external_kolmafia_namespaceObject.gametimeToInt)()))), "purple");
  (0,external_kolmafia_namespaceObject.print)("Profit earned: ".concat(formatNumber(fullResult.total), " meat"), "purple");

  if (globalOptions.printDetails) {
    var message = function message(head, meat, items) {
      var color = arguments.length > 3 && arguments[3] !== undefined ? arguments[3] : "black";
      return (0,external_kolmafia_namespaceObject.print)("".concat(head, " ").concat(formatNumber(meat + items), " meat, with ").concat(formatNumber(meat), " raw meat and ").concat(formatNumber(items), " from items"), color);
    ...
    snapshots.slice(1).forEach((snapshot, index) => {
      var taskSnapshot = snapshot.diff(snapshots[index]);
      var taskResult = taskSnapshot.value(external_kolmafia_namespaceObject.historicalPrice);
      if (taskResult.total !== 0) message("* ".concat(tasks[index].name, " generated"), taskResult.meat, taskResult.items);
    }); // List the top 3 gaining and top 3 losing items

    var losers = fullResult.itemDetails.sort((a, b) => a.value - b.value).slice(0, 3);
    var winners = fullResult.itemDetails.sort((a, b) => b.value - a.value).slice(0, 3);
    (0,external_kolmafia_namespaceObject.print)("Extreme items:", "blue");

    for (var _i = 0, _arr = [].concat(main_toConsumableArray(winners), main_toConsumableArray(losers.reverse())); _i < _arr.length; _i++) {
      var detail = _arr[_i];
      (0,external_kolmafia_namespaceObject.print)("".concat(formatNumber(detail.quantity), " ").concat(detail.item, " worth ").concat(formatNumber(detail.value), " total"));
    }
  }
}
})();
